...
export const validationRules = {
  required: (message?: string): Rule => ({
    required: true,
    message: message || 'Vui lòng nhập thông tin trường này !' || 'This field is required !',
  }),
  ...
  email: (message?: string): Rule => ({
    type: 'email',
    message: message || 'Vui lòng nhập Email hợp lệ !' || 'Invalid email !',
  }),
...
export const getFullUrlStatics = (path?: string): string => {
  if (!path) return ``;
  const host = env.api.baseUrl.service;
  return `${host}/statics/${path}`;
};

export const beautifyCurrency = (value: number): string => {
  const valueBiggerThanOneBillions = value.toString().length >= 10;
  const valueBiggerThanOneMillions = value.toString().length >= 7;
  const valueBiggerThanOneThousands = value.toString().length >= 4;

  let devider: number;
  let shortenCharacter: string;
  let newValue: string | number;
  if (valueBiggerThanOneBillions) {
    devider = 1000000000;
    shortenCharacter = 'B';
    newValue = (value / devider).toFixed(1);
  } else if (valueBiggerThanOneMillions) {
    devider = 1000000;
    shortenCharacter = 'M';
    newValue = (value / devider).toFixed(1);
  } else if (valueBiggerThanOneThousands) {
    devider = 1000;
    shortenCharacter = 'K';
    newValue = (value / devider).toFixed(1);
  } else {
  ...
  return `${newValue?.toString()} ${shortenCharacter}`;
};

export const getDatesFromTodayToNumberDay = (numberDay: number): number[] => {
  const today = new Date();
  const dataDates = [];
  for (let i = 0; i < numberDay; i += 1) {
    const setDate = moment(today).add(i, 'days');
    dataDates.push(setDate.valueOf());
  }
  return dataDates;
};

export const convertDaysOfWeek = (data: string): string[] => {
  const newData = (data || '').split(',');
  const newDayOfWeek = newData?.map((item: string) => {
    if (item === '7') return '0';
    return item;
  });
  return newDayOfWeek;
